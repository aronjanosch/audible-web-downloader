...
- **config/audible.db** (SQLite) - Primary store for accounts (including `authenticated` and `pending_invitation_token` for single-account invites), libraries, per-account library API cache (`library_cache`), download/book state (`books`), download queue (`download_queue`, `download_batches`), auto-download rules, and local scan results (`scan_cache`). Created and migrated on startup (`utils/db.py`).
- **config/settings.json** - Application settings including naming patterns and the general family-sharing `invitation_token` (not in SQLite)
- **config/auth/{account_name}/auth.json** - Audible OAuth tokens per account (files on disk; unchanged by the DB refactor)
- **downloads/** - Temporary download directory for AAX files, vouchers, and working files
- **Legacy JSON (migration only)** - If present from older installs, `config/accounts.json`, `config/libraries.json`, `config/library.json`, `config/download_queue.json`, and `library_data/` scan data are read once when the DB is first created, then left as backup copies; the running app reads and writes **audible.db** via `ConfigManager`, `LibraryManager`, queue managers, and `LibraryStorage`.

### Family sharing and the database

General invite links still validate against `invitation_token` in **settings.json**. Account rows created or updated through invite flows and OAuth completion are persisted in the **`accounts`** table in **audible.db** (same dict-shaped API as before: `ConfigManager.get_accounts()` / `save_accounts()`).
...
3. Saves authentication tokens to `config/auth/{account_name}/auth.json`
...
**Single-account invite tokens** — Stored per account in SQLite (`accounts.pending_invitation_token`) via `ConfigManager`; cleared after successful OAuth.
...
### State Management
- **Persistent state tracking** in SQLite (`download_queue`, `books`, etc.) with transactional updates
...
- **State persistence**: Download progress and batch state in **audible.db** (`download_queue`, `download_batches`); book download history in the **`books`** table (`LibraryManager`)
